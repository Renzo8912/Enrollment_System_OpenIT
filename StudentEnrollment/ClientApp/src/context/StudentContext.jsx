...
export const StudentProvider = ({ children }) => {
  ...
  const initialFormState = { id: null, firstName: '', lastName: '', program: 'Computer Science', year: '1', gender: 'Female', grade: '' };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
        ...
        program: { programName: studentForm.program },
        studentGrades: studentForm.grade !== '' ? [{ grade: parseInt(studentForm.grade) }] : []
    };
  ...
  const handleEdit = (std) => {
    let editGrade = '';
    if (std.studentGrades && std.studentGrades.length > 0) {
        editGrade = std.studentGrades[0].grade ?? std.studentGrades[0].Grade ?? '';
    }
    setStudentForm({ ...std, program: std.program, grade: editGrade });
